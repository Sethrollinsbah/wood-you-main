...
<div class="panel clearfix">
	...
	<div class="exc-settings">
		<label class="inline-label">
			{l s='Display this hook' mod='amazzingblog'}
		</label>
		<div class="inline-block">
			<select name="exceptions_type">
				<option value="1">{l s='on all pages except checked' mod='amazzingblog'}</option>
				<option value="2"{if $settings.type == 2} selected{/if}>{l s='only on checked pages' mod='amazzingblog'}</option>
			</select>
		</div>
		<a href="#" class="chk-action checkall">{l s='Check all' mod='amazzingblog'}</a>
		<a href="#" class="chk-action uncheckall">{l s='Unheck all' mod='amazzingblog'}</a>
		<a href="#" class="chk-action invert">{l s='Invert selection' mod='amazzingblog'}</a>
	</div>
	<div class="exc-list">
