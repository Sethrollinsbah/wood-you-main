...
<section id="boninstagram">
    <div class="instagram-carousel-container clearfix block {if $display_caroucel}bonhidden{/if}">
        <div class="instagram-home-title revealOnScroll animated fadeInUp" data-animation="fadeInUp">
            <h3>{l s='Instagram' mod='boninstagramcarousel'}</h3>
            <span>{l s='Direct message us on our instagram for offers.' mod='boninstagramcarousel'}</span>
        </div>
        <div class="block_content">
            <ul class="instagram-list {if $display_caroucel}owl-carousel-instagram owl-carousel owl-theme owl-loaded owl-drag {else}clearfix{/if}">
                {for $i=1 to $limit}
                    <li class="instagram-item {if !$display_caroucel}instagram-item-gallery{/if} revealOnScroll animated fadeInUp" data-animation="fadeInUp" >
                        <a href="https://www.instagram.com/{if $instagram_type == "tagged"}explore/tags/{$user_tag|escape:'htmlall':'UTF-8'}{else}{$user_id|escape:'htmlall':'UTF-8'}{/if}/" target="_blank" rel="nofollow">
                            <img src="{$baseurl|escape:'htmlall':'UTF-8'}img/parseImg/sample-{$i|escape:'htmlall':'UTF-8'}.jpg" alt="instagram-{$i|escape:'htmlall':'UTF-8'}">
...
{*                                                    <span class="instagram_likes">{rand(1,20)}</span>*}
{*                                                    <span class="instagram_comment">{rand(1,50)}</span>*}
                        </a>
                    </li>
                {/for}
            </ul>
