...
class WishList extends ObjectModel
{
    ...
    public function delete()
    {
        Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'wishlist_email` WHERE `id_wishlist` = '.(int)($this->id)
        );
        Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'wishlist_product` WHERE `id_wishlist` = '.(int)($this->id)
        );

        if ($this->default) {
            $result = Db::getInstance()->executeS(
                'SELECT * FROM `'._DB_PREFIX_.'wishlist` WHERE `id_customer` = '.
            ...
            foreach ($result as $res) {
                Db::getInstance()->update(
                    'wishlist',
                    array('default' => '1'),
                    'id_wishlist = '.(int)$res['id_wishlist']
                );
            }
        }
    ...
    * Get Customers having a wishlist
    ...
    public static function getProductCountByIdCustomer($id_customer)
    {
        if (!Validate::isUnsignedId($id_customer) || $id_customer < 1) {
            die(Tools::displayError());
        }

        return Db::getInstance()->getValue(sprintf(
            'SELECT COUNT(*)
            FROM `'._DB_PREFIX_.'wishlist` w
            LEFT JOIN `'._DB_PREFIX_.'wishlist_product` wp ON (wp.`id_wishlist` = w.`id_wishlist`)
            WHERE w.`id_customer` = %d
              AND wp.`id_product` IS NOT NULL',
            (int)$id_customer
        ));
    }

    public static function getProductIdsByIdCustomer($id_customer, $default_wishlist = false)
    {
        $products = array();

        if (Validate::isUnsignedId($id_customer) && $id_customer > 0) {
            $sql = '';
            ...
            $result = Db::getInstance()->executeS(sprintf(
                'SELECT wp.`id_product`
                FROM `%1$swishlist` w
                LEFT JOIN `%1$swishlist_product` wp ON (wp.`id_wishlist` = w.`id_wishlist`)
                WHERE w.`id_customer` = %2$d
                  %3$s
                  AND wp.`id_product` IS NOT NULL
                GROUP BY wp.`id_product`',
                _DB_PREFIX_,
                (int)$id_customer,
                $sql
            ));
    ...
    public static function addProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute, $quantity)
    {
        ...
        if (empty($result) === false && sizeof($result)) {
            ...
            return (Db::getInstance()->execute(
                'INSERT INTO `'._DB_PREFIX_.'wishlist_product` (
                    `id_wishlist`,
                    `id_product`,
    ...
    */
    public static function isDefault($id_customer)
    {
        return (bool)Db::getInstance()->getValue(
            'SELECT * FROM `'._DB_PREFIX_.'wishlist` WHERE `id_customer` = '.(int)$id_customer.' AND `default` = 1'
        );
    }
    ...
    */
    ...
    {
        Db::getInstance()->update('wishlist', array('default' => 0), 'id_customer = '.(int)$this->id_customer);

        return Db::getInstance()->update('wishlist', array('default' => 1), 'id_wishlist = '.(int)$this->id);
    }
};
