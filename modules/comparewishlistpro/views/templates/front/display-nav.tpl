...
<div class="easywishlist-block-nav d-xs-none d-md-flex">
  <a href="{$product_comparison_page_url|escape:'html':'UTF-8'}" title="{l s='Compare products' mod='comparewishlistpro'}">
    {if $appearance == 'all' || $appearance == 'icon'}{include file="./comparison-icon.tpl"}{/if} {if $appearance == 'all' || $appearance == 'text'}<span>{l s='Compare products' mod='comparewishlistpro'}</span>{/if}<span class="cwp-count compare-products-count">{$product_comparison_count|intval}</span>
  </a>
</div>
{/if}
