...
$(document).ready(function(){
	...
	$(document).on('change', 'select.exc', function() {
		...
		e.preventDefault();
		$(this).closest('.form-horizontal').attr('class', 'form-horizontal '+$(this).data('type'));
	...
	$(document).on('change', 'select[name="settings[carousel][type]"]', function(){
		$(this).closest('.carousel').toggleClass('no-scripts', $(this).val() != 1);
	});
	...
	$(document).on('change', '#carousel_type', function(){
		...
		$('.p-option').toggleClass('hidden', itemType != 'p');
		$('.special, .special_option').addClass('hidden');
		$('.special_option.'+carouselType).removeClass('hidden').closest('.special').removeClass('hidden');
		$('.not-for-some').parent().removeClass('hidden');
		$('.not-for-some.not-for-'+carouselType).parent().addClass('hidden');
