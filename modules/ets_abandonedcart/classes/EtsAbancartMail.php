...
class EtsAbancartMail extends MailCore
{
        ...
        try {
            ...
            if ($isMailApi && $mailApiType) {
                ...
                if ($mailMsg) {
                    ...
                    return false;
                $apiKeyId = $mailApiType !== 'hotmail' ? Configuration::get('ETS_ABANCART_MAIL_API_KEY_' . Tools::strtoupper($mailApiType), null, null, $id_shop) : null;
                switch ($mailApiType) {
                    case 'sendgrid' :
    ...
    /* ----- Outlook -----*/
    private static function getOutlookConfig()
    {
    ...
    public static function sendOutlookMail(
        ...
        if (!self::isAccessTokenValid()) {
            $accessToken = self::refreshOutlookAccessToken();
            if (!$accessToken) {
    ...
    private static function refreshOutlookAccessToken()
    {
        $config = self::getOutlookConfig();
        $data = [
    ...
    private static function saveAccessToken($response)
    {
