...
{ 
    ...
    {        
        ...
        foreach($configs as $key => $config)
        {
            if($config['type']=='file' && $this->$key && @file_exists(_PS_ETS_MM_IMG_DIR_.$this->$key) && !Ets_megamenu::imageExits($this->$key,$this->id))
    ...
    public function saveData()
    {
        ...
        if($this->fields['form']['name']=='menu')
        {
            ...
            if(isset($bubble_text_entered) && $bubble_text_entered)
            {
                ...
                if(!$bubble_background_color || !Validate::isCleanHtml($bubble_background_color))
                    $errors[] = Ets_megamenu::$trans['bubble_background_color_is_required'];
            }
            
        }
        if($this->fields['form']['name']=='tab')
        {
            $link_type = Tools::getValue('link_type');
            switch($link_type)
            {
                ...
                foreach($configs as $key => $config)
                {
                    ...
                    elseif($config['type']=='file')
                    {
                        ...
                        if(isset($_FILES[$key]['tmp_name']) && isset($_FILES[$key]['name']) && $_FILES[$key]['name'])
                        {
                            ...
                            $imageName = @file_exists(_PS_ETS_MM_IMG_DIR_.Tools::strtolower($_FILES[$key]['name']))|| Tools::strtolower($_FILES[$key]['name'])==$this->$key ? $salt.'-'.Tools::strtolower($_FILES[$key]['name']) : Tools::strtolower($_FILES[$key]['name']);
                            ...
                            if(!Validate::isFileName($_FILES[$key]['name']))
                                $errors[] = $config['label'].' '.Ets_megamenu::$trans['field_not_valid']; 
                            ...
                            {                                    
                    			...
                    			)
                    			{
                                    ...
                                    if(!$errors)
                                    {
                                        if($this->$key!='')
                                        {
                                            $oldImage = _PS_ETS_MM_IMG_DIR_.$this->$key;
                                    ...
                                    $errors[] = $config['label'].' '.Ets_megamenu::$trans['file_valid_type'];
                            }
                        }
                        //End upload file                       
                    }
