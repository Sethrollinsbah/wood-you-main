...
class Ets_opc_additionalinfo_field extends ObjectModel
{
    ...
    public static function getListField($id_lang = null,$active=false){
        ...
        $activeCondition = $active ? 'f.enable=1' : '';
        $langCondition = $id_lang ? 'fl.id_lang=' . (int)$id_lang : '';
        $query = new DbQuery();
        $query->select('f.*, f.id_ets_opc_additionalinfo_field as id, fl.title, fl.description, fl.id_lang, fl.options')
            ->from('ets_opc_additionalinfo_field', 'f')
            ->innerJoin('ets_opc_additionalinfo_field_lang','fl','f.id_ets_opc_additionalinfo_field = fl.id_ets_opc_additionalinfo_field')
            ->where('f.deleted = 0')
            ->where('f.id_shop = 0 OR f.id_shop = ' . $idShop)
