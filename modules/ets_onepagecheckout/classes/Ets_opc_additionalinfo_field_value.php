...
class Ets_opc_additionalinfo_field_value extends ObjectModel
{
    ...
    public function delete()
    {
        if(parent::delete())
        {
            if($this->value && file_exists(_PS_ETS_OPC_UPLOAD_DIR_.$this->value))
            {
                ...
                if($field->type=='file' && Validate::isFileName($this->value))
                    Ets_opc_tools::unlink(_PS_ETS_OPC_UPLOAD_DIR_.$this->value,_PS_ETS_OPC_UPLOAD_DIR_);
    ...
    public static function customerDownloadFile($file_name)
    {
        ...
        if(($file = Db::getInstance()->getRow($sql)) && $file['value'] && Validate::isFileName($file['value']) &&  file_exists(_PS_ETS_OPC_UPLOAD_DIR_.$file['value']))
        {
            $file_url = Validate::isFileName($file['value']) ? _PS_ETS_OPC_UPLOAD_DIR_.$file['value']:'';
            if($file_url)
            {
