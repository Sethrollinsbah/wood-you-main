...
class LightOpenID
{
    ...
    public function __set($name, $value)
    {
        switch ($name) {
        case 'identity':
            if (\Tools::strlen($value = trim((String) $value))) {
                if (preg_match('#^xri:/*#i', $value, $m)) {
                    $value = \Tools::substr($value, \Tools::strlen($m[0]));
    ...
    protected function set_realm($uri)
    {
        ...
        $realm .= (($end = strpos($uri, '/', $offset)) === false) ? $uri : \Tools::substr($uri, 0, $end);
    ...
    protected function request_curl($url, $method='GET', $params=array(), $update_claimed_id)
    {
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, 'LightOpenID');
        ...
        if ($method == 'HEAD' && curl_getinfo($curl, CURLINFO_HTTP_CODE) == 405) {
            curl_setopt($curl, CURLOPT_HTTPGET, true);
            $response = curl_exec($curl);
            $response = \Tools::substr($response, 0, strpos($response, "\r\n\r\n"));
        }
        ...
        if ($method == 'HEAD' || $method == 'GET') {
            ...
            if ($method == 'GET') {
                $header_response = \Tools::substr($response, 0, strpos($response, "\r\n\r\n"));
            }
            ...
            foreach (explode("\n", $header_response) as $header) {
                $pos = strpos($header, ':');
                if ($pos !== false) {
                    $name = \Tools::strtolower(trim(\Tools::substr($header, 0, $pos)));
                    $headers[$name] = trim(\Tools::substr($header, $pos+1));
                }
            }
    ...
    protected function request_streams($url, $method='GET', $params=array(), $update_claimed_id)
    {
        ...
        case 'HEAD':
            ...
            if (!empty($headers)) {
                if ((int)\Tools::substr($headers[0], \Tools::strlen('HTTP/1.1 ')) == 405) {
        ...
        $data = \Ets_opc_tools::file_get_contents($url, false, $context);
	    ...
	    if (!empty($data)){
		    $http_response_header = json_decode($data,true);
    ...
    protected function get_provider_name($provider_url)
    {
        ...
        if (!empty($provider_url)) {
            ...
            $result = \Tools::strtolower(
                (count($tokens) > 1 && \Tools::strlen($tokens[1]) > 3)
                    ? $tokens[1]
    ...
    protected function getAxAttributes()
    {
        ...
        if ($alias = $this->getNamespaceAlias('http://openid.net/srv/ax/1.0', 'ax')) {
            ...
            $length = \Tools::strlen('http://axschema.org/');
            
            foreach (explode(',', $this->data['openid_signed']) as $key) {
                $keyMatch = $alias . '.type.';
                
                if (strncmp($key, $keyMatch, \Tools::strlen($keyMatch)) !== 0) {
                    continue;
                }
                
                $key = \Tools::substr($key, \Tools::strlen($keyMatch));
                $idv = $prefix . '_value_' . $key;
                $idc = $prefix . '_count_' . $key;
                $key = \Tools::substr($this->getItem($prefix . '_type_' . $key), $length);
    ...
    private function getNamespaceAlias($namespace, $hint = null)
    {
        ...
        if (empty($hint) || $this->getItem('openid_ns_' . $hint) != $namespace) {
            ...
            $prefix = 'openid_ns_';
            $length = \Tools::strlen($prefix);
            
            foreach ($this->data as $key => $val) {
                if (strncmp($key, $prefix, $length) === 0 && $val === $namespace) {
                    $result = trim(\Tools::substr($key, $length));
                    break;
                }
            }
        } else {
