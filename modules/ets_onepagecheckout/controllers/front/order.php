...
class Ets_onepagecheckoutOrderModuleFrontController extends ModuleFrontController
{
    ...
    public function _initContent()
    {
        ...
        if($group->price_display_method)
            ...
            $tax=true;
        $id_address = $this->context->customer->isLogged() ?  Ets_opc_db::getLastCustomerAddressId($this->context->customer->id):0;
                ...
                foreach($option_list as $key => &$option)
                {
                   ...
                   if($option['carrier_list']) 
                   {
                        foreach($option['carrier_list'] as $id_carrier=> $carrier)
                        {
                            ...
                            if (isset($carrier['instance']) && Validate::isLoadedObject($carrier['instance'])) {
                                if ($moduleId = Module::getModuleIdByName($carrier['instance']->external_module_name)) {
                                    $carrier['id'] = $carrier['instance']->id;
                                    $delivery['module_name'] = $carrier['instance']->external_module_name;
    ...
    public function _submitCustomerLogin()
    {
        ...
        } elseif (method_exists('Validate','isPasswd') && !self::isPasswd($passwd)) {
            $field_errors[] = array(
                'field'=> 'customer_login_password',
    ...
    public function _submitCompleteMyOrder()
    {
        ...
        if($additional_fields && is_array($post_values) && Ets_onepagecheckout::validateArray($post_values))
        {   
            foreach($additional_fields as $field)
            {
                if($field['required'])
                {
                    ...
                    if($field['type']=='file' && (!isset($_FILES['additional_info']['name'][$field['id']]) || (isset($_FILES['additional_info']['name'][$field['id']]) &&!$_FILES['additional_info']['name'][$field['id']])))
                    {
                        $field_errors[] = array(
                            'field' => 'additional_info_'.$field['id'],
                            'error' => sprintf($this->module->l('%s is required','order'),$field['title'])
        ...
        else
        {
            if($type_checkout_options=='guest')
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if(method_exists('Validate','isPasswd') && !self::isPasswd($customer_guest['password']))
                            {
                                $field_errors[] = array(
                                    'field'=> 'customer_guest_password',
                                    'error' => $this->module->l('Email is invalid','order'),
            ...
            if($type_checkout_options=='create')
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if(isset($customer_create['password']) && $customer_create['password'])
                        {
                            if(method_exists('Validate','isPasswd') &&   !self::isPasswd($customer_create['password']))
                            {
                                $field_errors[] = array(
                                    'field'=> 'customer_create_password',
            ...
            if($additional_fields)
            {
                foreach($additional_fields as $field)
                {
                    ...
                    if($field['type']=='file' && isset($_FILES['additional_info']['name'][$field['id']]) && isset($_FILES['additional_info']['size'][$field['id']]) && ($file_name = $_FILES['additional_info']['name'][$field['id']]))
                    {
                        $file_errors = array();
                        $this->module->validateFile($file_name,$_FILES['additional_info']['size'][$field['id']],$file_errors);
                        if($file_errors)
                        {
        ...
        if($additional_fields && !$this->errors)
        {
            foreach($additional_fields as $field) 
            {
                ...
                {
                    $file = $this->module->uploadFile(isset($_FILES['additional_info']) ? $_FILES['additional_info']:[], $field['id'],$this->errors);
                    if($id_ets_opc_field_value = Ets_opc_additionalinfo_field_value::getIdByField($field['id'],$this->context->cart->id))
                    {
                        $fieldValue = new Ets_opc_additionalinfo_field_value($id_ets_opc_field_value);
                    }
                    ...
                    $file_old = Validate::isFileName($fieldValue->value) ? $fieldValue->value:'';
                    $fieldValue->value = $file ? :'';
                    $fieldValue->file_name = isset($_FILES['additional_info']['name'][$field['id']]) ? $_FILES['additional_info']['name'][$field['id']]: '';
                    if($fieldValue->id)
                    {
                        if(!$fieldValue->update())
                        {
                            if($fieldValue->value && Validate::isFileName($fieldValue->value) && file_exists(_PS_ETS_OPC_UPLOAD_DIR_.$fieldValue->value))
                                Ets_opc_tools::unlink(_PS_ETS_OPC_UPLOAD_DIR_.$fieldValue->value,_PS_ETS_OPC_UPLOAD_DIR_);
                            $this->errors[] = sprintf($this->module->l('Saving %s failed','order'),$field['title']);
                        }elseif($file_old && Validate::isFileName($file_old)  && file_exists(_PS_ETS_OPC_UPLOAD_DIR_.$file_old))
                            Ets_opc_tools::unlink(_PS_ETS_OPC_UPLOAD_DIR_.$file_old,_PS_ETS_OPC_UPLOAD_DIR_);
                    }
                    elseif(!$fieldValue->add())
                    {
                        if($fieldValue->value && Validate::isFileName($fieldValue->value) && file_exists(_PS_ETS_OPC_UPLOAD_DIR_.$fieldValue->value))
        ...
        if(!$this->errors)
        {
            if($type_checkout_options=='guest')
            {
                ...
                if($customer_guest && is_array($customer_guest) && Ets_onepagecheckout::validateArray($customer_guest))
                {
                    ...
                    $passwd = isset($customer_guest['password']) && $customer_guest['password'] ? $customer_guest['password'] :   time();
    ...
    public function checkCaptcha()
    {
        ...
        if(Configuration::get('ETS_OPC_CAPTCHA_ENABLED') && Configuration::get('ETS_OPC_CAPTCHA_TYPE'))
        {
            ...
            else
            {
                ...
                if ($recaptcha) {
                    $response = json_decode(Ets_opc_tools::file_get_contents($link_capcha,false,null,5,false,null,[],['google.com']), true);
