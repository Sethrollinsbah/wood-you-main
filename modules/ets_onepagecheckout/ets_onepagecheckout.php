...
class Ets_onepagecheckout extends Module
{
    ...
    public function install()
	{
        if(self::isInstalled('etsextracustomerfields')){
            throw new PrestaShopException($this->l("The module ets_extracustomerfields has been installed"));
        }
            ...
            foreach ($languages as $language) {
                if (isset($language['iso_code']) && ($language['iso_code'] != 'en' || $language['iso_code'] != 'he')) {
                    if (($new_dir = dirname(__FILE__) . '/mails/' . $language['iso_code'])) {
                        $this->recurseCopy($temp_dir_ltr, $new_dir,dirname(__FILE__) . '/mails/');
                    }
                }
            }
        }
    ...
    public function recurseCopy($src, $dst,$allowed_directory=null)
    {
        if(!$allowed_directory)
            $allowed_directory = dirname(__FILE__);
        if (!@file_exists($src) || !Ets_opc_tools::isPathInAllowedDirectory($src,$allowed_directory))
            return false;
        $dir = opendir($src);
        if (!@is_dir($dst))
            @mkdir($dst);
        while (false !== ($file = readdir($dir))) {
            if (($file != '.') && ($file != '..')) {
                if (is_dir($src . '/' . $file)) {
                    $this->recurseCopy($src . '/' . $file, $dst . '/' . $file,$allowed_directory);
    ...
    public function hookDisplayHeader()
    {
        ...
        if($fc=='module' && $module==$this->name && $controller=='order')
        {
            ...
            $this->context->controller->registerJavascript('ets-js-jquery-plugins-timepicker', 'js/jquery/plugins/timepicker/jquery-ui-timepicker-addon.js', ['position' => 'bottom', 'priority' => 90]);
    ...
    public function getContent()
    {
        ...
        if(Tools::isSubmit('delImage') && ($name = Tools::getValue('delImage')) && $name=='ETS_OPC_SAFE_ICONS')
        {
            ...
            if(Validate::isFileName($image))
            {
                ...
                if(Validate::isFileName($image) && file_exists(_PS_ETS_OPC_IMG_DIR_.$image))
                    Ets_opc_tools::unlink(_PS_ETS_OPC_IMG_DIR_.$image,_PS_ETS_OPC_IMG_DIR_);
        ...
        foreach($settings as $input) {
            ...
            else
            {
                if($input['type']=='file')
                {
                    ...
                    elseif(isset($_FILES[$input['name']]) && isset($_FILES[$input['name']]['name'])  && $_FILES[$input['name']]['name'] && isset($_FILES[$input['name']]['size'])  && $_FILES[$input['name']]['size'])
                    {
                    ...
                    else
                    {
                        ...
                        elseif($setting['type']=='file')
                        {
                            if(isset($_FILES[$name]['tmp_name']) && isset($_FILES[$name]['name']) && $_FILES[$name]['name'])
                            {
                                ...
                                $fileName = Validate::isFileName($imageName) ?  _PS_ETS_OPC_IMG_DIR_.$imageName:'';
                                if (isset($temp_name) && file_exists($temp_name))
                                    Ets_opc_tools::unlink($temp_name);
                                if($fileName && move_uploaded_file($_FILES[$name]['tmp_name'], $fileName))
                                {
                                    if(($fileOld=Configuration::get($name)) && Validate::isFileName($fileOld) )
                                    {
                                        $oldImage = _PS_ETS_OPC_IMG_DIR_.$fileOld;
                                        if(Validate::isFileName($fileOld) && file_exists($oldImage))
                                            Ets_opc_tools::unlink($oldImage,_PS_ETS_OPC_IMG_DIR_);
                                    }
    ...
    public function getSettingsField($render_form = false)
    {
        ...
        return array(
            ...
            array(
                ...
                'default'=>0,
                'desc' => $this->l('This option is designed to help you safely test your checkout process without affecting your live store. Only users with specified IP addresses will be able to view and interact with the checkout page created by the module. When you enable this option, the remaining users with other IP addresses will use the default checkout page of PrestaShop.'),
                'values' => array(
    				array(
    					'id' => 'active_on',
    					'value' => 1,
            ...
            array(
                ...
                'tab'=>'social_login',
                'desc' => $this->l('When logging in with a social account, if no account is linked to that email, a new account will be created, and the password will be emailed to the customer. This password can be used for future logins and will only be sent once.')
            ),
            array(
    ...
    public function createUser($_profile, $provider)
	{
		if (!$_profile) {
		...
		elseif ($provider)
		{
			$profile = $this->prepareDataToSave($_profile);
    ...
    {
        $css_content = Ets_opc_tools::file_get_contents(dirname(__file__).'/views/css/color.css');
    ...
    public function uploadFile($files,$id_field,&$errors)
    {
        ...
        if(isset($files['tmp_name'][$id_field]) && isset($files['name'][$id_field]) && $files['name'][$id_field])
        {
            ...
            if ( !Validate::isFileName($files['name'][$id_field]) || !move_uploaded_file($files['tmp_name'][$id_field], _PS_ETS_OPC_UPLOAD_DIR_.$files['name'][$id_field]))
    ...
    public function getListFieldsAddress()
    {
        $fields = array(
         ...
         if(Configuration::get('ETS_OPC_POSITION_FIELDS_ADDRESS'))
         {
            ...
            $positions = array('alias','firstname','lastname','company','address','address2','city','state','postcode','country','phone','phonemobile','vatnumber','dni','other');
    ...
    public function copy_directory($src, $dst,$allowed_directory=null)
    {
        if(!$allowed_directory)
            $allowed_directory =_PS_UPLOAD_DIR_;
        if(!is_dir($src) || !Ets_opc_tools::isPathInAllowedDirectory($src,$allowed_directory))
            return '';
        $dir = opendir($src);
        if(!file_exists($dst))
            @mkdir($dst);
    ...
    public function rrmdir($dir,$allowed_directory=null)
    {
        if(!$allowed_directory)
            $allowed_directory =_PS_UPLOAD_DIR_;
        if(!Ets_opc_tools::isPathInAllowedDirectory($dir,$allowed_directory))
            return '';
        $dir = rtrim($dir, '/');
        if ($dir && is_dir($dir)) {
                ...
                foreach ($objects as $object) {
                    if ($object != "." && $object != "..") {
                        ...
                        elseif(file_exists($dir . "/" . $object))
                            Ets_opc_tools::unlink($dir . "/" . $object,$allowed_directory);
                    }
                }
            }
            rmdir($dir);
    ...
    private function checkOverrideDir()
    {
        if (defined('_PS_OVERRIDE_DIR_')) {
            ...
            // Xác thực rằng base là một thư mục hợp lệ
            if (!is_dir($base)) {
                $this->safeMkDir($base);
            }
