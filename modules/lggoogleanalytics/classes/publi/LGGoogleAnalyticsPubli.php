...
class LGGoogleAnalyticsPubli
{
    ...
    public static $modules = [
        'lgcookieslaw' => [
            ...
            'description' => [
                ...
                'es' => 'Módulo actualizado para cumplir plenamente con la normativa referente a cookies de 2022. Permite bloquear las cookies hasta obtener el consentimiento del usuario y seleccionar el tipo de cookies a instalar entre funcionales y de terceros.',
                'fr' => 'Module mis à jour pour se conformer pleinement à la réglementation en matière de cookies de 2022. Il vous permet de bloquer les cookies jusqu’à ce que vous obteniez le consentement de l’utilisateur qui accède à votre site Web.',
            ],
            'url' => [
    ...
    public static function renderHeader()
    {
        ...
        return Context::getContext()->smarty->fetch(
            self::$module->getLocalPath() . 'views/templates/admin/header.tpl'
        );
    }
    ...
    public static function renderFooter()
    {
        ...
        return Context::getContext()->smarty->fetch(
            self::$module->getLocalPath() . 'views/templates/admin/footer.tpl'
        );
    }

    public static function getReadme()
    {
        $path = _MODULE_DIR_ . self::$module->name . '/readme/';
        $file = $path . 'readme_' . Context::getContext()->language->iso_code . '.pdf';
        $file_en = $path . 'readme_en.pdf';

        if (file_exists($file)) {
            return self::$module->getPathUri() . 'readme/readme_' . Context::getContext()->language->iso_code . '.pdf';
        } elseif (file_exists($file_en)) {
            return self::$module->getPathUri() . 'readme/readme_en.pdf';
        }
