...
{   
    ...
    public function renderListMedios(){
        ...
        $this->fields_list = array(
            ...
            'id_decidir' => array(
                'title' => $this->l('ID payment in  Decidir'),
                'width' => 140,
                'type' => 'text',
                'align' => 'center',63
            ),    
        ...
        //arreglar esto!!!!!!
        $urlAddBank = AdminController::$currentIndex.'&configure=decidir&section=4&add_mediopago&token='.Tools::getAdminTokenLite('AdminModules').$this->modulesParams; 

        $helper->title = 'Medios de pago ';
        ...
        $helper->currentIndex = AdminController::$currentIndex.'&configure=decidir&section=4';
    ...
    public function updateMediosPago($ArrayMedioPagofields){
        ...
        $query = 'UPDATE `'._DB_PREFIX_.'medios` SET name="'.$ArrayMedioPagofields['name'].'", type="'.$tipoName.'", id_decidir='.$ArrayMedioPagofields['id_decidir'].', active='.$ArrayMedioPagofields['active'].' WHERE id_medio = '.$ArrayMedioPagofields['id_medio'];        if(!Db::getInstance()->execute($query)){
