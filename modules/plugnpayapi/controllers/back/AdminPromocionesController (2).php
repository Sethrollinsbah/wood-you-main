...
{	
    ...
    public function renderListPromociones(){
        ...
        $this->fields_list = array(
            ...
            'entity' => array(
                'title' => $this->l('Financial institution'),
                'width' => 60,
                'type' => 'text',
                'align' => 'center',
            ),
            ...
            'send_installment' => array(
                'title' => $this->l('Installment to send'),
                'width' => 50,
                'type' => 'text',
                'align' => 'center',
            ),
            'coeficient' => array(
                'title' => $this->l('Koefficient'),
                'width' => 20,
                'type' => 'text',
                'align' => 'center',
            ),      
        ...
        $urlAddpromo = AdminController::$currentIndex.'&configure=decidir&section=6&add_promocion&token='.Tools::getAdminTokenLite('AdminModules').$this->modulesParams; 
        
        $helper->title = 'Promociones';

        $helper->table = (isset($this->name)? $this->name: "").'_promocion';
        $helper->show_toolbar = true;
        $helper->toolbar_scroll = true;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->currentIndex = AdminController::$currentIndex.'&configure=decidir&section=6';
    ...
    public function getById($idPromocion){
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'promociones WHERE id_promocion='.pSQL($idPromocion);
        $result = Db::getInstance()->ExecuteS($sql);
    ...
    public function updatePromocionVisible($idPromocion){
        ...
        $query = 'UPDATE '._DB_PREFIX_.'promociones SET active = IF (active, 0, 1) WHERE id_promocion='.pSQL($idPromocion);

        if(!Db::getInstance()->execute($query)){
            die('Error de actualizacion.');        
        } else {
            return $this->displayConfirmation($this->l('Settings updated').'<br/>');
        }
    }

    public function updatePromocion($ArrayPromocionfields, $idpromo){
        ...
        if($ArrayPromocionfields['id_installment'] == '' || $ArrayPromocionfields['id_installment'] == '0')
                $ArrayPromocionfields['id_installment'] = '0';            
        
        if($ArrayPromocionfields['send_installment'] == '' || $ArrayPromocionfields['send_installment'] == '0')
        ...
        $query = 'UPDATE `'._DB_PREFIX_.'promociones` SET name="'.pSQL($ArrayPromocionfields['plan_name']).'", payment_method='.pSQL($ArrayPromocionfields['payment_method']).', entity= '.pSQL($ArrayPromocionfields['entity']).', send_installment="'.pSQL($ArrayPromocionfields['send_installment']).'", days="'.pSQL($daysList).'", init_date="'.pSQL($ArrayPromocionfields['date_from']).'", final_date="'.pSQL($ArrayPromocionfields['date_to']).'", installment="'.pSQL($InstallmentList).'", coeficient='.pSQL($ArrayPromocionfields['coeficient']).', discount='.pSQL($ArrayPromocionfields['discount']).', reimbursement='.pSQL($ArrayPromocionfields['reinbursement']).' WHERE id_promocion = '.pSQL($ArrayPromocionfields['id_promocion']);         
        if(!Db::getInstance()->execute($query)){
            die('Error de actualizacion.');        
        }
    }
    ...
    public function insertPromocion($ArrayPromocionfields){
        ...
        if($ArrayPromocionfields['reinbursement'] == '' )
                $ArrayPromocionfields['reinbursement'] = '0';    
        if ($ArrayPromocionfields['entity'] == '')
                $ArrayPromocionfields['entity'] = 1;
        $query = 'INSERT INTO `'._DB_PREFIX_.'promociones` (name, payment_method, entity, send_installment, days, init_date, final_date, installment, coeficient, discount, reimbursement, active) VALUES("'.pSQL($ArrayPromocionfields['plan_name']).'", '.pSQL($ArrayPromocionfields['payment_method']).', '.pSQL($ArrayPromocionfields['entity']).' , "'.pSQL($ArrayPromocionfields['send_installment']).'", "'.pSQL($daysList).'", "'.pSQL($ArrayPromocionfields['date_from']).'", "'.pSQL($ArrayPromocionfields['date_to']).'", "'.pSQL($InstallmentList).'", '.pSQL($ArrayPromocionfields['coeficient']).', '.pSQL($ArrayPromocionfields['discount']).', '.pSQL($ArrayPromocionfields['reinbursement']).', '.pSQL($ArrayPromocionfields['active']).')';
