...
<div class="post-item-wrapper{if $col} col-sm-{$col|intval}{if $k % $settings.col_num == 0} first-in-line{/if}{/if}">
	<div class="post-item{if !empty($settings.show_date)} has-date{/if}">
		...
		{if !empty($settings.title_truncate)}
			<h3 class="post-item-title{if $settings.display_type == 'grid' || $settings.display_type == 'carousel'} overflow-ellipsis{/if}">
				{if !empty($settings.link_title)}<a href="{$post.link|escape:'html':'UTF-8'}">{/if}
			...
			<div class="post-item-footer clearfix">
				<div class="post-item-infos pull-left">
					...
					{if !empty($settings.show_comments)}
						<a href="{$post.link|escape:'html':'UTF-8'}#post-comments" class="post-item-info comments-num">
		...
		{if !empty($settings.cover_type)}
			...
			{if !empty($cover_src)}
				<div class="post-item-cover">
					{if !empty($settings.link_cover)}<a href="{$post.link|escape:'html':'UTF-8'}">{/if}
		...
		{if !empty($settings.truncate)}
			<div class="post-item-content">
				<div class="post-item-content">{$post.content|strip_tags|truncate:$settings.truncate:'...'|escape:'html':'UTF-8'}
				</div>
				{if !empty($settings.truncate) && !empty($settings.show_readmore)}
					<a href="{$post.link|escape:'html':'UTF-8'}" title="{l s='Read more' d='Shop.Theme.Global'}" class="item-readmore butt btn_border mini">
