...
<div id="{$carousel.identifier|escape:'html':'UTF-8'}" class="easycarousel theme-carousel {$carousel.settings.tpl.custom_class|escape:'html':'UTF-8'}{if $in_tabs} ec-tab-pane{else} carousel_block{/if}{if !empty($custom_class)} {$custom_class|escape:'html':'UTF-8'}{/if}{if $is_17 && empty($carousel.name) && $carousel.settings.carousel.n} nav_without_name{/if} {if $c_settings.type == 2} scroll-wrapper{/if}">
	{if !$in_tabs && !empty($carousel.name)}
		<div class="col-12">
			<h3 class="title_main_section title_block carousel_title">
				<span>{$carousel.name|escape:'html':'UTF-8'}</span>
			</h3>
		</div>
	{/if}
			...
			{foreach array_values($carousel.items) as $k => $column_items}
				...
				{if $k}</div>{/if}<div class="c_col">
					{foreach $column_items as $i}
						{$type = $carousel.type}
						<div class="c_item {if $type != 'suppliers' && $type != 'manufacturers' && $type != 'categories' && $type != 'subcategories'}product-item{/if}">
	...
	{if !empty($carousel.view_all_link)}
		<div class="text-center">
			<a href="{$carousel.view_all_link|escape:'html':'UTF-8'}" class="view_all">{l s='View all' d='Shop.Modules.Easycarousels'}</a>
		</div>
	{/if}
</div>
{* since 2.5.1 *}
