...
<div class="thumbnail-container">
    ...
    {block name='product_image'}
        {if $settings.image_type != '--'}
            ...
            <a href="{$product.url}" class="thumbnail product-thumbnail scale_image" itemprop="url">
                <img src="{$product.cover.bySize.$image_type.url}" alt="{$product.cover.legend}" class="{if !empty($product.second_img_src)}primary-image{/if}">
                {if !empty($product.second_img_src)}
                    <img src="{$product.second_img_src}" class="img-responsive secondary-image" itemprop="image">
                {/if}
            </a>
        {/if}
        {/block}
        {block name='product_stickers'}
            {if !empty($settings.stock) && $product.availability_message}
                <div class="product-flags label_{$product.availability}">
                    <div class="product-availability {$product.availability}">
    ...
    <div class="right-block">
        ...
        <div class="product-description">
            ...
            <div class="button-container">
                ...
                {if $settings.add_to_cart || $settings.displayProductListFunctionalButtons}
                    <form type="post" action="{$cart_page_url}" class="product-item-buttons">
                        {if $settings.add_to_cart}
                            ...
                            <span>{l s='Add to cart' d='Shop.Modules.Easycarousels'}</span>
                            </button>
                        {/if}
                ...
                {if $settings.quick_view || $settings.view_more}
                    <div class="highlighted-informations">
                        {if $settings.quick_view}
                            <a href="#" class="quick-view function-btn hidden-sm-down" data-link-action="quickview">{*{l s='Quick view' d='Shop.Modules.Easycarousels'}*}</a>
                        {/if}
                        {if $settings.view_more}
                            <a href="{$product.url}" class="lnk_view function-btn">{*{l s='More' d='Shop.Modules.Easycarousels'}*}</a>
                        {/if}
                    </div>
                {/if}
                {/block}
